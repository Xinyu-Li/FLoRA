# FLoRA engine: Using Analytics to Measure and Facilitate Learners' own Regulation Activities
...
The repository also includes the following Markdown documents:

- `SQL_FILES_MIGRATION_SUMMARY.md` - Summarizes the migration of SQL initialization files into `src/main/resources/sql/` so they can be packaged inside the application JAR.
- `DATABASE_INITIALIZATION.md` - Explains the automatic database initialization workflow, configuration options, SQL file lookup order, and troubleshooting guidance.
- `SOFTWARE_REQUIREMENTS.md` - Lists the main software prerequisites and setup notes for running and developing the FLoRA Backend application.
- `PYTHON_SOFTWARE_REQUIREMENTS.md` - Provides deployment and dependency guidance for the Python-based chat services, including required packages, language models, and environment variables.
