...
function render() {
    let processVisualToolHtml = `
        ...
        <div id="collapseProcessVisual" class="my-horizontal-collapse-tools my-processVisual">
            <div class="card card-body" style="height:100%; min-height: 200px; overflow:auto; display: flex; flex-direction: column;">
                <div class="header-container" style="display: flex; justify-content: space-between; align-items: center;">
                    ...
                    <!-- <div id="processVisualTextArea" style="text-align: left; border: 1px solid black; padding: 5px;">
                        <p id="fixedTextLine" style="margin: 0;">${processVisualSetGoalLabel} :</p>
                            <ol style="margin: 0; padding-left: 20px;" id="student-goal-list"></ol>
                    </div>-->
                    <!-- 关闭按钮 -->
...
<!--                <div id="process_visual_container1_previous" style="width: 100%; max-height: 20vh; margin: 0; padding: 0;"></div>-->
                <!-- 第二个柱状图容器 -->
                <div id="process_visual_container2_current" style="width: 100%; flex: 1; min-height: 0; margin: 0; padding: 0;"></div>
                <!-- 关闭按钮 -->
...
let productGoalDisplay = document.querySelector("#product-goal-display"); //TODO lilin
let processGoalDisplay1 = document.querySelector("#process-goal1-display"); //TODO lilin
let processGoalDisplay2 = document.querySelector("#process-goal2-display"); //TODO lilin
...
function processQuestionResponse(data) {
    ...
    updateTextProductDisplay(converted_goal_text_list);
    updateTextProcessDisplay(converted_goal_text_list);
...
// function updateTextProductDisplay(data){
//
//     productGoalDisplay.innerHTML = "";
//     if (data.length > 1) {
//         data.forEach((sentence, index) => {
//             if (index > 0) {
//
//                 // 👇 New: also render into product-goal-display
//                 const line = document.createElement("div");
//                 line.textContent = sentence;
//                 productGoalDisplay.appendChild(line);
//             }
//         });
//     } else {
//         const msg = "No Goal Detected!!!";
//
//         // New: product goal fallback
//         productGoalDisplay.textContent = msg;
//     }
// }

function updateTextProductDisplay(data) {
    // Clear existing content safely (no innerHTML)
    while (productGoalDisplay.firstChild) {
        productGoalDisplay.removeChild(productGoalDisplay.firstChild);
    }

    if (Array.isArray(data) && data.length > 0) {
        const line = document.createElement("div");
        line.textContent = data[0]; // 👈 only show the first item
        productGoalDisplay.appendChild(line);
    } else {
        const msg = document.createElement("div");
        msg.textContent = "No Goal Detected!!!";
        productGoalDisplay.appendChild(msg);
    }
}

function updateTextProcessDisplay(data){

    while (processGoalDisplay1.firstChild) {
        processGoalDisplay1.removeChild(processGoalDisplay1.firstChild);
    }
    if (Array.isArray(data) && data.length > 1) {
        const line = document.createElement("div");
        line.textContent = data[1]; // 👈 only show the first item
        processGoalDisplay1.appendChild(line);
    } else {
        const msg = document.createElement("div");
        msg.textContent = "No Goal Detected!!!";
        processGoalDisplay1.appendChild(msg);
    }
    while (processGoalDisplay2.firstChild) {
        processGoalDisplay2.removeChild(processGoalDisplay2.firstChild);
    }
    if (Array.isArray(data) && data.length > 1) {
        const line = document.createElement("div");
        line.textContent = data[2]; // 👈 only show the first item
        processGoalDisplay2.appendChild(line);
    } else {
        const msg = document.createElement("div");
        msg.textContent = "No Goal Detected!!!";
        processGoalDisplay2.appendChild(msg);
    }
}
