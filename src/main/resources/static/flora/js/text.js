(function (global) {
    ...
    TextHighlighter.prototype.doHighlight = function (keepRange) {
        ...
        // console.log("---------------------");
        currentRange = range; //点击button之后会给currentRange 重复赋值，需要在if block里面置成null
        // saveRange = range;

        if (whetherDoHighlight) {
            // console.log("do highlight");
            whetherDoHighlight = false;
            currentRange = null; //
        } else {
            // console.log("not do highlight");
            return;
        }
        ...
        if (this.options.onBeforeHighlight(range) === true) {
            ...
            // handle 合并的过程，如果不需要合并可去掉，但是需要处理多层span
            // normalizedHighlights = this.normalizeHighlights(createdHighlights);

            // this.options.onAfterHighlight(range, normalizedHighlights, timestamp); //此选项会合并highlight

            this.options.onAfterHighlight(range, createdHighlights, timestamp);
            // console.log(createdHighlights); // 获取highlight text 并处理

            saveHighlight = createdHighlights;

            saveHighlightText = "";
            for (let i = 0; i < saveHighlight.length; i++) {
                // console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^");
                // console.log(saveHighlight[i].parentNode);
                // console.log(saveHighlight[i].firstChild);
                // console.log(saveHighlight[i].lastChild);
                // console.log(saveHighlight[i].nextSibling);
                // console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^");
                saveHighlightText += saveHighlight[i].lastChild.textContent;
                // console.log(saveHighlightText);
            }
        }
    ...
    TextHighlighter.prototype.serializeOneHighlight = function (tempHighlight) {
        var highlights = tempHighlight,
            refEl = this.el,
            hlDescriptors = [];
