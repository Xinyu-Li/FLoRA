...
function activatePanel(panel) {
    ...
    const toolPanels = [
        ...
        typeof collapsePlanner2     !== 'undefined' ? collapsePlanner2     : null,
        typeof collapseSearch     !== 'undefined' ? collapseSearch     : null,
        typeof collapseGPTScaffolds     !== 'undefined' ? collapseGPTScaffolds     : null,
        typeof collapseGptChatWithScaffolds     !== 'undefined' ? collapseGptChatWithScaffolds     : null,
    ].filter(Boolean);
...
function activatePanelGeneral(panel) {
    ...
    const toolPanels = [  // 根据study的需要增加别的
        ...
        typeof collapseGPTScaffolds     !== 'undefined' ? collapseGPTScaffolds     : null,
        typeof collapseGptChatWithScaffolds     !== 'undefined' ? collapseGptChatWithScaffolds     : null,
        typeof collapseQuizConfidence     !== 'undefined' ? collapseQuizConfidence     : null,
        typeof collapseMetaJudgements     !== 'undefined' ? collapseMetaJudgements     : null,
    ].filter(Boolean);

    const essayPanel    = typeof collapseWriteEssay !== 'undefined' ? collapseWriteEssay : null; // 便于引用
    ...
    if (isEssayClick) {
        if (essayOpen) {                   // 2-A 折叠 essay
            ...
            if (openTool) {                  // 确保另一工具待在右侧
                ...
                essayPanel.classList.add('in-tools');
            }
        }
        return;
    }
    ...
    if (panelWasOpen) {
        ...
        if (essayOpen) {
            essayPanel.classList.add('in-tools');
            essayPanel.classList.remove('in-tools-move-more-left');
        }
        return;
    }
