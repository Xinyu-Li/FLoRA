...
//    public static String ETHERPAD_API_KEY = "49a6a6329b12c93fa4eff0cfe53257bba064637fe277c1c9a4535d24fa5ef81e";
    ...
    public static final Map<String, String> COURSE_LANGUAGE_MAP = new ConcurrentHashMap<>();
...
//"https://study.floralearn.cn",
    ...
    public static final String[] myOrigins = {"http://localhost", "http://localhost:80", "http://localhost:8088", "http://localhost:8080", "http://localhost:8081", "http://localhost:4200", "http://localhost:3000",
        ...
        "https://gaila.hku.hk", "https://infoseeking.floraengine.org","https://flora.pku.edu.cn", "https://asia.floraengine.org", "https://flora.foi.hr", "https://sz.floraengine.org"};
...
//    public static Map<String, String> URL_COURSE_ID_MAP = new HashMap<>();
    ...
    public static void clearCourseLanguageMap() {
        COURSE_LANGUAGE_MAP.clear();
    }

    public static void registerCourseLanguage(String courseId, String language) {
        if (hasText(courseId) && hasText(language)) {
            COURSE_LANGUAGE_MAP.put(courseId.trim(), language.trim());
        }
    }

    public static void registerCourseLanguages(String language, String... courseIds) {
        if (!hasText(language) || courseIds == null) {
            return;
        }
        for (String courseId : courseIds) {
            registerCourseLanguage(courseId, language);
        }
    }

    public static String resolveLanguageByCourseId(String courseId) {
        if (!hasText(courseId)) {
            return LANGUAGE;
        }
        return COURSE_LANGUAGE_MAP.getOrDefault(courseId.trim(), LANGUAGE);
    }
