...
//            "WHERE qz.name like CONCAT('%', #{quizName}, '%') AND qz.course = #{courseId} AND qa.userid = #{userId} AND qat.questionsummary like '%Have you previously participated to this study%' AND qat.responsesummary like '%Yes%';")
    @Select("SELECT qas.slot as question_slot, qasd.value as selected_value " +
            "from moodle.mdl_quiz q " +
            "        join moodle.mdl_quiz_attempts qa on qa.quiz = q.id " +
            "        join moodle.mdl_question_attempts qas on qas.questionusageid = qa.uniqueid " +
            "        join moodle.mdl_question qt on qt.id = qas.questionid " +
            "        join moodle.mdl_question_attempt_steps qast on qast.questionattemptid = qas.id " +
            "        join moodle.mdl_question_attempt_step_data qasd on qasd.attemptstepid = qast.id and qasd.name = 'answer' " +
            "where q.name like CONCAT('%', #{quizName}, '%') and q.course = #{courseId} and qa.userid=#{userId} and qa.attempt=1 order by question_slot limit 1;")
    ...
    @Select(
            "SELECT " +
            "q.name as quiz_name, q.course as course_id, qa.userid as user_id, " +
            "qas.slot as question_slot, qas.questionsummary as question_summary, " +
            "qas.responsesummary as response_summary, qasd.value as selected_value " +
            "from mdl_quiz q " +
            ...
            "where q.name like CONCAT('%', #{quizName}, '%') and " +
            "      q.course = #{courseId} and qa.userid=#{userId} and qa.attempt=( " +
            "        SELECT MAX(qa2.attempt) " +
            "        FROM mdl_quiz_attempts qa2 " +
            "        WHERE qa2.quiz=q.id AND qa2.userid=#{userId} " +
            "    ) order by question_slot;" //# 只取最新的 attempt
    )
    ...
    /*@Select("<script>" +
                    ...
                    "#{item}" +
                    "</foreach>" +
                    " and qa.userid=#{userId} and qa.attempt=( " +
                    "        SELECT MAX(qa2.attempt) " +
                    "        FROM mdl_quiz_attempts qa2 " +
                    "        WHERE qa2.quiz=q.id AND qa2.userid=#{userId} " +
                    "    ) order by question_slot;" +
                    "</script>")
    List<MdlQuizVO> getQuizResponseByUserIdAndCourseIdListAndQuizName(@Param("quizName") String quizName, @Param("courseIdList") List<String> courseIdList, @Param("userId") Long userId);
*/
