...
public interface MdlFeedbackMapper extends BaseMapper<MdlFeedback> {
    ...
    @Select("SELECT f.course as course_id, f.name as feedback_name, fc.userid as user_id, fi.name as item_name, fi.id as item_id, fv.value FROM mdl_feedback f JOIN mdl_feedback_completed fc ON f.id = fc.feedback JOIN mdl_feedback_value fv ON fc.id = fv.completed JOIN mdl_feedback_item fi ON fv.item = fi.id WHERE f.name like CONCAT('%', #{feedbackName}, '%') AND f.course = #{courseId} AND fc.userid = #{userId} AND fi.hasvalue = 1")
