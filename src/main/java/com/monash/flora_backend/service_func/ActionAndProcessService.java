...
//        }
        ...
        for(String temp : strings) {
            if (StrUtil.isEmpty(temp) || temp.startsWith("null")) {
                continue;
            }
            TraceData tempTraceData = new TraceData();
...
//            log.info("tempTraceData id:" + tempTraceData.getId());
    ...
    private List<String> findRealTimeSrlProcess(List<TraceDataRealTimeProcess> traceDataList, List<String> srlProcessPatternList, String modelType, List<TraceDataRealTimeProcess> needUpdateList, Map<String, String> srlProcessPatternRegexMap) {
        ...
        if (allActionsStr.length() > 50_000) {
            log.info("allActionsStr is large (length={}), using dedicated thread for regex matching (real-time)", allActionsStr.length());
            List<TraceDataRealTimeProcess> threadNeedUpdateList = new ArrayList<>();
            ...
            Thread regexThread = new Thread(null, () -> {
                try {
                    resultRef.set(findRealTimeSrlProcess(allActionsStr, srlProcessPatternList, modelType, threadNeedUpdateList, srlProcessPatternRegexMap));
                } catch (RuntimeException e) {
                    exceptionRef.set(e);
                }
            }, "srl-regex-realtime-large-input", 32 * 1024 * 1024);
            regexThread.start();
            try {
                regexThread.join();
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                log.error("Thread interrupted during real-time SRL process regex matching", e);
                return new ArrayList<>();
            }
            if (exceptionRef.get() != null) {
                throw exceptionRef.get();
            }
            needUpdateList.addAll(threadNeedUpdateList);
    ...
    public List<String> findSrlProcess(List<TraceData> traceDataList, List<String> srlProcessPatternList, String modelType, List<TraceData> needUpdateList, Map<String, String> srlProcessPatternRegexMap) {
        ...
        // When allActionsStr is very large, Java's recursive regex engine can cause StackOverflowError.
        // Run regex matching in a dedicated thread with a larger stack size to avoid this.
        if (allActionsStr.length() > 50_000) {
            log.info("allActionsStr is large (length={}), using dedicated thread for regex matching", allActionsStr.length());
            List<TraceData> threadNeedUpdateList = new ArrayList<>();
            AtomicReference<List<String>> resultRef = new AtomicReference<>();
            AtomicReference<RuntimeException> exceptionRef = new AtomicReference<>();
            Thread regexThread = new Thread(null, () -> {
                try {
                    resultRef.set(findSrlProcess(allActionsStr, srlProcessPatternList, modelType, threadNeedUpdateList, srlProcessPatternRegexMap));
                } catch (RuntimeException e) {
                    exceptionRef.set(e);
                }
            }, "srl-regex-large-input", 32 * 1024 * 1024); // 32MB stack
            regexThread.start();
            try {
                regexThread.join();
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                log.error("Thread interrupted during SRL process regex matching", e);
                return new ArrayList<>();
            }
            if (exceptionRef.get() != null) {
                throw exceptionRef.get();
