...
public class EssayServiceImpl extends ServiceImpl<EssayMapper, Essay> implements IEssayService {
    ...
    @Override
    public EssayVO findLatestVersionByUserIdAndCourseIdList(Long userId, List<String> courseIdList) {
        log.info("here>>>> findLatestVersionByUserIdAndCourseIdList{}   {}", userId, courseIdList);
        QueryWrapper<Essay> wrapper = new QueryWrapper<>();
        wrapper.eq("user_id", userId).in("course_id", courseIdList).orderByDesc("save_time").last("LIMIT 1");
        Essay essay = super.getOne(wrapper);
        return essay == null ? null : MyBeanCopyUtils.copyBean(essay, EssayVO.class);
    }
