...
//    }
    ...
    private void clearTraceDataBuffer() {
        ...
        // 过滤掉无效记录（userId 或 saveTime 为空）
        traceDataBuffer.removeIf(t -> t.getUserId() == null || t.getSaveTime() == null);
        ...
        try {
            ...
            log.error("Exception happen in iTraceDataService.saveBatch", e);
...
//    }
    ...
    private void checkNonCloseToolsEvent(TraceDataVO traceDataVO) {
        ...
        tryOutToolKeys.forEach(ty-> {
            ...
            if (traceDataVOOpen == null || traceDataVOOpen.getUserId() == null) {
                log.warn("OPEN event not found in Redis: {}", ty);
                return; // 跳过这个key
            }
            ...
            iGlobalCache.del(ty); // 删除已经处理完的OPEN 事件
        });
    }
...
//            log.info("Kafka listener (before): " + traceDataVO.toString());
            // 过滤无效记录
            if (traceDataVO == null || traceDataVO.getUserId() == null || traceDataVO.getSaveTime() == null) {
                return;
            }
...
//                    iGlobalCache.del(toolAccessTimesKey + "_OPEN");
                ...
                //如果从redis 中获取的 open trace是null，则不添加到traceDataBuffer中去
                if (traceDataVOOpen == null || traceDataVOOpen.getUserId() == null) {
                    log.warn("OPEN event not found in Redis for key: {}", toolAccessTimesKey + "_OPEN");
                } else {
                    traceDataBuffer.add(MyBeanCopyUtils.copyBean(traceDataVOOpen, TraceData.class));
                }
...
//                }
            ...
            if (traceDataVO.getSource().equals("ESSAY") && traceDataVO.getPageEvent().equals("WRITE_SENTENCE")) {
                ...
                if (!Objects.equals(MyConstant.SRL_MODEL, "copes")) { // 如果不是copes，不需要进行分类
                    ...
                    CompletableFuture<List<String>> futCls = batcher.submit(writeSentence, traceDataVO.getCourseId());
